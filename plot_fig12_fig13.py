...
for scenario in ["CTL", "EXP"]:
    ...
    for ocn_model in loaded_ocn_models:
        ...
        atm_filename = "data/hierarchy_average/%s/atm.nc" % (casename,)
        ocn_filename = "data/hierarchy_average/%s/ocn_regrid.nc" % (casename, )
...
for i, ocn_model in enumerate(ocn_models):
    ...
    mappable = _ax.contourf(lon, lat, SST_DIF,  clev_SST,  cmap=cmap_SST, transform=data_proj, zorder=1)#, extend="both")
    ...
    CS = _ax.contour(lon, lat, WVEL_DIF,  clev_W, transform=data_proj, colors="black", linewidths=1, zorder=5)
    ...
    #if ocn_model == "POP2":
        CS = _ax.contourf(lon, lat, NHFLX_DIF, [-1000, -1, 1, 1000], colors="none", hatches=["//", None, ".."], transform=data_proj, zorder=6)
    ...
    qv_ek   = _ax.quiver(lon[::skip_x], lat[::skip_y], UVEL_EK_DIF[::skip_y, ::skip_x], VVEL_EK_DIF[::skip_y, ::skip_x], pivot="tail", color="lime", scale_units="inches", scale=2, transform=data_proj, zorder=10, width=0.005)

    qv_wind = _ax.quiver(lon[::skip_x], lat[::skip_y], Usfc_DIF[::skip_y, ::skip_x], Vsfc_DIF[::skip_y, ::skip_x], pivot="tail", color="black", scale_units="inches", scale=2, transform=data_proj, zorder=11, width=0.005)

    qvk_ek   = plt.quiverkey(qv_ek,   X=0.82, Y=0.2,  U=1,    label="$ \\vec{v}_{\\mathrm{EK}}$ $ 1 \\, \\mathrm{cm} / \\mathrm{s}$", labelcolor="black", labelsep=0.15, coordinates="figure")
    qvk_wind = plt.quiverkey(qv_wind, X=0.82, Y=0.3,  U=1,  label="Surface wind $ 1 \\, \\mathrm{m} / \\mathrm{s}$", labelcolor="black", labelsep=0.15, coordinates="figure")
...
# Load total OHT
with Dataset("data/hierarchy_statistics/CTL_21-120/EMOM_CTL/ocn_analysis_OHT.nc", "r") as f_CTL:
    with Dataset("data/hierarchy_statistics/EXP_81-180/EMOM_EXP/ocn_analysis_OHT.nc", "r") as f_EXP:
