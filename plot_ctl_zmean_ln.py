...
output_dir = "graph"
plot_vars = ["SST", "PREC_TOTAL", "TAUX", "FSNT", "FLNT", "TAUY"]
...
fig, ax = plt.subplots(len(plot_vars), 1, sharex=True, figsize=(6, 4), constrained_layout=True, squeeze=False)
...
for (i, varname) in enumerate(plot_vars):
    ...
    for exp_name, caseinfo in sim_casenames.items():
        ...
        ax[i].plot(lat, _CTL_mean, linestyle=ls, color=lc, label=label)
...
fig.savefig("graph/CTL_zmean_precip.png", dpi=600)
plt.show()
plt.close(fig)
