...
for scenario in ["CTL",]:
    ...
    for exp_name, caseinfo in sim_casenames.items():
        ...
        filename = "data/%s/atm_analysis_%s.nc" % (casename, EOF_MODE)
...
for exp_name, caseinfo in sim_casenames.items():
    ...
    for m in range(plotted_modes):
        _ax = fig.add_subplot(spec[ax_idx[1], m], **proj_kw)
        ax.append(_ax)
...
fig.savefig("%s/EOF_map_%s.png" % (output_dir, EOF_MODE), dpi=300)
plt.show()
plt.close(fig)
