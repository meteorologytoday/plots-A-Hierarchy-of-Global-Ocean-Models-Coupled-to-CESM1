    ...
    for exp_name, caseinfo in sim_casenames.items():
        ...
        for varname, filename  in sim_var.items():
            ...
            data[scenario][exp_name][varname]  = {
                "MAM" : np.mean(_d[2:5,  :, :], axis=0),
                "SON" : np.mean(_d[8:11, :, :], axis=0),
            }
...
fig.savefig("%s/SSTA_correlation.png" % (output_dir,), dpi=300)
plt.show()
plt.close(fig)
