...
plot_info = {
    ...
    "PSL" : {
        "display"   : "std(PSL)",
        "unit"      : "[${}^\\circ\\mathrm{hPa}$]",
        "cmap_mean" : "hot_r",
        "clevels_plain"       : np.linspace(0, 10, 11),
        "clevels_plain_tick"  : np.linspace(0, 5, 6),
        "clevels_diff" : np.linspace(-1, 1,  11) * 1.5,
        "clevels_resp" : np.linspace(-0.2, 0.2,  11),
        "factor"        : 1e-2,
    },
...
fig.savefig("%s/PSL_variance_%s.png" % (output_dir, plot_type), dpi=300)
plt.show()
plt.close(fig)
