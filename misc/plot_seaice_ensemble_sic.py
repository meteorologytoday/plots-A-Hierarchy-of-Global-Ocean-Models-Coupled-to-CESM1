...
if model_set == "CCSM4":
    for i in range(6):
        filename = "data/seaice_ensemble/CCSM4_f09/sic_ens%02d.nc" % (i+1,)
        with Dataset(filename, "r") as f:
            vice.append(f.variables["sic"][:])

elif model_set == "CESM1":        
    for i in range(40):
        if i == 10:
            continue
        filename = "data/seaice_ensemble/CESM1_f09/sic_ens%02d.nc" % (i+1,)
        with Dataset(filename, "r") as f:
            vice.append(f.variables["sic"][:])
    ...
    for col in range(6):
        ...
        for i in range(len(vice)):
            if model_set == "CCSM4" and i == 0:
                ...
                linewidth = 1
                linestyle = "solid"
            _ax.contour(lon, lat, vice[i][m, :, :], [0.15, 0.5,], transform=ccrs.PlateCarree(), colors=colors, linewidths=linewidth, linestyles=linestyle, zorder=zorder)

            
        _ax.contour(lon, lat, vice_ens[m, :, :], [0.15, 0.5,], transform=ccrs.PlateCarree(), colors=["darkred", "darkviolet"], linewidths=2, linestyles="--", zorder=100)
        ...
        for i in range(len(vice)):
            if model_set == "CCSM4" and i == 0:
                ...
                linewidth = 1
                linestyle = "solid"
            _ax.contour(lon, lat, vice[i][m, :, :], [0.15,], transform=ccrs.PlateCarree(), colors=colors, linewidths=linewidth, linestyles=linestyle, zorder=zorder)

            
        _ax.contour(lon, lat, vice_ens[m, :, :], [0.15,], transform=ccrs.PlateCarree(), colors=["darkviolet"], linewidths=3, linestyles="--", zorder=100)
        """

        m += 1
    
fig.savefig("graph/seaice_ensemble_sic_%s.png" % (model_set,), dpi=600)
