...
#with Dataset("data/OGCM_SSTA_Statistics_remove_ENSO.nc", "r") as f:
#    OGCM_SSTA_noENSO_std = f.variables["SSTA_noENSO_STD"][:]


#data["CTL"]["OGCM_NOENSO"] = { 
#        "SST_SASTD" : OGCM_SSTA_noENSO_std,
#}

#sim_casenames["OGCM_NOENSO"] = {
#    'ax_idx' : (0, 5)
#}
...
plot_info = {
    "PREC_TOTAL" : {
        "display"   : "std(PREC)",
        "unit"      : "[$\\mathrm{mm} / \\mathrm{yr}$]",
        "cmap_mean" : "hot_r",
        "clevels_plain"       : np.linspace(0, 1, 11) * 2000,
        "clevels_plain_tick"  : np.array([0, 0.5, 1]) * 2000,
        "clevels_diff" : np.linspace(-1, 1,  11),
        "clevels_resp" : np.linspace(-0.2, 0.2,  11),
        "factor"        : 365*86400.0*1000,
    },
}["PREC_TOTAL"]
...
fig.savefig("%s/PREC_TOTAL_variance_%s.png" % (output_dir, plot_type), dpi=300)
plt.show()
plt.close(fig)
