...
for scenario in ["CTL", "EXP"]:
    ...
    for exp_name, caseinfo in sim_casenames.items():
        ...
        #data[scenario][exp_name]["NetRad_ZONAL_AM"] = data[scenario][exp_name]["FSNT_ZONAL_AM"] - data[scenario][exp_name]["FLNT_ZONAL_AM"]
...
fig, ax = plt.subplots(len(plot_vars), len(boxes), sharex=True, figsize=(8, 8), constrained_layout=True)


for j, (box_label, lon_rng) in enumerate(boxes):
            
    lon_idx = np.logical_and(lon > lon_rng[0], lon < lon_rng[1]) 
...
#        _ax.set_title("(%s) %s" % ( thumbnail[i], plot_info["display"]))
        ...
        for exp_name, caseinfo in sim_casenames.items():
            ...
            _CTL_mean = np.mean(data["CTL"][exp_name][var_mean][:, :, lon_idx], axis=(0, 2,)) * factor
            _EXP_mean = np.mean(data["EXP"][exp_name][var_mean][:, :, lon_idx], axis=(0, 2,)) * factor
            _diff_mean = _EXP_mean - _CTL_mean
            _ax.plot(lat, _diff_mean, linestyle=ls, color=lc, label=label)


        if 'yticks_mean' in plot_info:
            _ax.set_yticks(plot_info["yticks_mean"])
 
        if j == 0 :
            _ax.set_ylabel(r"[ %s ]" % plot_info["unit"])
        else:
            _ax.set_yticklabels([])

        _ax.set_ylim(plot_info["ylim_mean_diff"])
...
#ax[0, 0].legend(ncol=3, loc='upper left', framealpha=1, fontsize=12, columnspacing=1.0, handletextpad=0.3)
