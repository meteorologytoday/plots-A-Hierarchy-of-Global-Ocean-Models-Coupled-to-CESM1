...
OGCM_files = {
    "CTL" : "data/POP2_OHT/POP2_CTL_1-100.nc",
    "EXP" : "data/POP2_OHT/POP2_EXP_81-180.nc",
}
...
#    trans = transforms.blended_transform_factory(_ax.transData, _ax.transAxes)
    ...
    net_radtoa = net_radtoas[exp_name]
    dTdt = dTdts[exp_name]
    ...
    energy_text = "RAD: %.2f PW\nWKR: %.2f PW\nSTG: %.2f PW\nFRC: %.2f PW" % (net_radtoa, _DIFF_SRC_SNK, dTdt, - (_DIFF_SRC_SNK + net_radtoa) + dTdt)
    #_ax.text(-85, -.18, energy_text, ha='left', va='bottom', transform=_ax.transData, bbox=dict(facecolor="white", edgecolor="black", boxstyle='round'), fontsize=10)

    # _ax.text(-80, 0.11, exp_name, ha='left', va='center', transform=_ax.transData, bbox=dict(facecolor="white", edgecolor="black", boxstyle='round'), fontsize=10)
